...
// use crate::database::connect_db::DB_CLIENT;
...
//     use crate::database::connect_db::{connect_db, DB_CLIENT};
//     use crate::database::db_namespace::{RPBOT_DB_NAME, UNIVERSE_COLLECTION_NAME};
// 
//     static SERVER_ID: u64 = 1;
// 
//     async fn insert_universe() -> Result<(), String> {
//         connect_db().await;
...
//             creator_id: 0,
//         };
//         universe.insert_universe().await
//     }
// 
...
//         DB_CLIENT.lock().unwrap().clone().unwrap().database("RpBot");
//         delete_previously_setup().await;
//         match result {
//             Ok(data) => {
//                 match data {
//                     None => {assert!(false, "No data recovered")}
//                     Some(_) => {assert!(true)}
//                 }
//             }
