...
impl Universe{
    ...
    /// # Returns
    /// A MongoDB cursor over matching `Universe` documents.
    pub async fn get_creator_universes(user_id: u64) -> mongodb::error::Result<Cursor<Universe>> {
        let db_client = DB_CLIENT.lock().unwrap().clone();
    ...
    pub async fn add_server_to_universe(&self, server_id: u64) -> mongodb::error::Result<UpdateResult> {
        let db_client = DB_CLIENT.lock().unwrap().clone();
        let filter = doc! { "_id": self.universe_id };
        let data_to_insert = doc! {"$addToSet": { "server_ids": server_id.to_string()}};
        db_client.database(RPBOT_DB_NAME).collection::<Universe>(UNIVERSE_COLLECTION_NAME).update_one(filter, data_to_insert).await
    }
    ...
    pub async fn get_universe_by_id(universe_id: String) -> mongodb::error::Result<Option<Universe>> {
        let db_client = DB_CLIENT.lock().unwrap().clone();
        let object_id = ObjectId::parse_str(&universe_id)
            .map_err(|e| {println!("{}", e)});
        let filter = doc! { "_id": object_id.unwrap() };
        db_client.database(RPBOT_DB_NAME).collection::<Universe>(UNIVERSE_COLLECTION_NAME).find_one(filter).await
    }
}
...
mod test{
    ...
    async fn test_recover_universe_by_id() {
        ...
        let id = universe.unwrap().inserted_id.as_object_id().unwrap().to_hex();
